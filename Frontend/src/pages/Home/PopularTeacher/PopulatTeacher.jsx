...
const PopulatTeacher = () => {
    const [instructors, setInstructors] = useState([])
    const axiosFetch = useAxiosFetch()
    useEffect(()=> {
      axiosFetch.get('/popular-instructors').then((data) => {
        setInstructors(data.data)}).catch((err)=> {console.log(err)})
      },[])
    ...
    <div className='grid mb-28 md:grid-cols-2 lg:grid-cols-4 w-[90%] gap-4 mx-auto'>
      {
        instructors?.slice(0,4).map((instructor,i) =>(
          <div  key={i} className='flxe dark:text-white hover:-translate-y-2 duration-200 cursor-pointer 
